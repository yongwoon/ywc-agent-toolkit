...
4. **Branch does not exist, no owner recorded** → branch it from the current base, following the same push-at-creation idiom `codex/skills/ywc-sequential-executor/references/aggregate-pr.md` already uses for `$WORK_BRANCH`:
   ...
   ```
   then immediately call `python3 codex/skills/scripts/update-state.py wave-int-owner <N> --tip-sha "$(git rev-parse wave-int/<N>)"`.
...
1. Merge base **into** `wave-int/<N>` — never rebase, per `../references/pr-conflict-resolution.md`:
   ...
   ```
   then immediately call `python3 codex/skills/scripts/update-state.py wave-int-owner <N> --tip-sha "$(git rev-parse wave-int/<N>)"` (FR-5's second checkpoint — the branch's identity could have shifted here, not just its accumulating content).
