...
For each task whose Step 4e delivery completed (DONE) **and whose wave is not `Hardener-BLOCKED`** (i.e. the wave created no `wave-int/<N>`, or its `hardener_verdict` is not `BLOCKED` — check `.ywc-run-state.json`; a task whose wave is `Hardener-BLOCKED` skips this step per [references/wave-integration-branch.md](references/wave-integration-branch.md)):
