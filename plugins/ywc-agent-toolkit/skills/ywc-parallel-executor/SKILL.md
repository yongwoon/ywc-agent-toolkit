...
**4e.5. Wave-boundary Hardener and aggregation** — After every task in the wave has completed 4c.5, collect only sanitized mutation evidence at this boundary. Dispatch at most one Hardener attempt per eligible task, keyed by task identity and wave, only when remaining mutation work or survivors are established and either Cleaner was legitimately skipped or Cleaner returned `DONE`/`DONE_WITH_CONCERNS`. Never dispatch Hardener after Cleaner `BLOCKED` or `NEEDS_CONTEXT`, and never dispatch it for `N/A — no quality gate contract` or `report-only`. The Hardener may edit only `ownership.test_fixture_paths` and `test_fixture_symbols`; it may not edit production code or adjudicate a survivor equivalent. Its bounded result includes status, score/evidence, attempts, every residual survivor, and changed paths, with sanitized evidence only. Its canonical attempt cap is three; survivors remain reportable after the cap. Before delivery, aggregate all task and gate results at the wave boundary with fixed precedence `BLOCKED > NEEDS_CONTEXT > DONE_WITH_CONCERNS > DONE`. Retain all sanitized concerns, unavailable-tool gaps, residual survivors, and evidence references from every task; a later `DONE` must never mask an earlier concern. Under `advisory`, an unavailable authorized tool or capped survivor is `DONE_WITH_CONCERNS`; under `enforced`, it is `BLOCKED`; under `report-only`, record the gap without mutation. A missing or unverifiable packet/evidence boundary is always `NEEDS_CONTEXT`. The aggregate has reporting authority only: it cannot stage, commit, push, create or merge a PR, deliver changes, or authorize work outside each task's Ownership. Any task not permitted to proceed remains in its original worktree and branch, and the existing rerouting, triage, cleanup skip, and Completion Report behavior applies unchanged.
