...
3. **Confirm design context (REQUIRED).** You cannot infer this from code.
   ...
   → else invoke the `impeccable` skill's teach mode if available → else ask the three questions inline). Do not
...
- **With impeccable**: delegate to the `impeccable` skill's audit mode (or
...
1. **Commit to a bold direction.** From the Phase 0 context, choose one clear
   ...
   projects.
2. **Apply the engine.**
   - **With impeccable**: use the `impeccable` skill's craft mode for a new surface, or
     its polish mode to refine the existing one.
...
`codexdocs/design-slop-audit-{YYYY-MM-DD}.md`):
