...
description: >-
  (ywc) Use when executing tasks from the tasks/ directory in strict order
  through their full lifecycle. Triggers: "execute task", "implement tasks
...
| `--resume-disposition` | `resume\|stop` | | Required when an authoritative checkpoint exists and the invocation scope differs or resume needs an explicit decision. |
...
**Procedure**: at Pre-flight, read `.codex/settings.local.json` for `ywDevSequentialExecutor.externalSpecUrls`. A valid `deny`, `allow`, or `allowlist` policy is used silently. Missing or invalid policy returns `NEEDS_CONTEXT` with the exact settings key; do not prompt or create configuration. For `allowlist`, require a non-empty list of canonical HTTPS origins.
...
**Long-range compaction**: carry a one-line task status digest per completed task (for example, `000027-020 DONE local-merge <merge-sha>`), and treat `.ywc-run-state.json` plus task artifacts (`README.md`, `task.md`, `tasks/completed/<task>/`) as the durable source of truth after compaction.
...
When executing a range, transitions never pause for user progress or approval. Resume, branch/worktree conflict, CI wait/timeout, and external URL policy branches return bounded terminal statuses (`NEEDS_CONTEXT`, `BLOCKED`, or `DONE_WITH_CONCERNS`) and do not emit prompts. The shared rule, full stop list, force-continue rule, zero-output rule, and the "non-stop transitions ≠ non-stop coding on one branch" reasoning live in [`../references/non-stop-execution.md`](../references/non-stop-execution.md).
...
The escalation conditions remaining in this skill's scope — Spec Reference conflict (Step 1b), verification first failure with unclear cause (Step 4), Stop Condition borderline (Step 3) — are defined in [references/advisor-escalation.md](./references/advisor-escalation.md). Merge conflict and CI first-failure escalations (originally conditions 3 and 4 in that reference) move to `ywc-finish-branch`'s scope at Step 5; finish-branch consumes its own `advisor_budget: 1` for those escalations independently of this skill's budget of 3. After successful advisor dispatch, apply [`subagent-async-monitoring.md`](../references/subagent-async-monitoring.md) at 300 seconds; monitor only the returned target and consume the existing slot.

Advisor unavailable fallback preserves ownership and never redispatches: spec conflict → `NEEDS_CONTEXT` with the conflict evidence; ambiguous verification failure → `BLOCKED` with the failing output; borderline Stop Condition → honor the stop and return `BLOCKED`; Pattern C plan review → `NEEDS_CONTEXT` with the bounded plan evidence. A possibly-live target blocks lifecycle actions until quiescence or an attributable terminal result.
...
**`--draft` PR health sweep**: After finish-branch returns with the draft PR URL, run the bot review polling loop from [`../references/pr-bot-polling.md`](../references/pr-bot-polling.md), then invoke `ywc-handle-pr-reviews` for this PR regardless of `BOT_COUNT == 0`. The handler checks review artifacts, CI status, and merge-readiness; a zero bot-comment count is not terminal success. The PR stays as draft after review response — do not un-draft or merge.
...
Merge conflict during pull → return `BLOCKED` with `branch_conflict`; do not prompt. PR-vs-base conflict at delivery → `ywc-finish-branch`'s Merge-Readiness Gate (Step 4 final, per [../references/pr-conflict-resolution.md](../references/pr-conflict-resolution.md)) handles it: a merely-behind branch is auto-updated and CI re-verified; a real textual conflict returns `BLOCKED`. CI timeout (>30 min) → return `DONE_WITH_CONCERNS` with `ci_timeout`; never ask whether to continue waiting. CI failure → `ywc-finish-branch` Step 4 diagnoses the failing checks, categorizes the failure (lint/format, type, test, build), and applies up to 2 fix attempts automatically; returns `BLOCKED` only when fixes are exhausted — do not stop at the first CI failure. `gh` CLI not authenticated → return `NEEDS_CONTEXT` with the missing auth prerequisite. Task not found or dirty working tree → return `NEEDS_CONTEXT` with a bounded missing prerequisite; never prompt.
