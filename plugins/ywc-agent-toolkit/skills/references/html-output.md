# HTML Output Format for Codex Skills
...
Review skills already share the five-level severity vocabulary defined in
[symbols.md](symbols.md) §3. HTML mode maps each tier to one CSS custom
property so that a finding tagged Critical looks identical across
`ywc-impl-review`, `ywc-security-audit`, `ywc-spec-validate`, and
`ywc-product-review`.

| symbols.md tier | Symbol | CSS token | Color (oklch) |
| --- | --- | --- | --- |
...
The symbol from `symbols.md` is kept *inside* each finding card as well — the
color is the scan layer, the symbol is the plain-text fallback that survives a
copy-to-Markdown. Do not invent new tiers or colors; the five-level scale is the
...
```markdown
| `--format` | `--format markdown\|html` | Output format. Default `markdown`. With `html`, writes a self-contained HTML report to `claudedocs/`. See [html-output.md](../references/html-output.md). |
```

**One paragraph appended to the skill's existing `## Output Format` section:**
