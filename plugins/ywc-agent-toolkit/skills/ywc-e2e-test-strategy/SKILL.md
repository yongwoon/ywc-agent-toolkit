...
description: >-
  (ywc) Use when designing an automated E2E strategy for a web app, setting
  up Playwright, auditing E2E coverage gaps, or generating Playwright tests
  for a critical user flow with CI integration. Triggers: "E2E 테스트 전략",
  "Playwright 설정", "e2e test strategy", "playwright setup", "playwright
  test", "E2Eテスト戦略", "Playwright設定". Do not use for manual
  verification sheets (use ywc-gen-testcase), unit/integration test
  implementation, or security-focused penetration testing (use
  ywc-security-audit).
---
