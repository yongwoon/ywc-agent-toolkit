...
For the full Data Model self-check, run the review checklist in [../references/schema/core.md](../references/schema/core.md) Part C and the stack-specific notes for the project's ORM or DDL style.
...
> spec 초안이 완성됐습니다. `ywc-spec-ready`를 실행해서 자동으로 검증·수렴 루프(Critical/Warning 해소 → DONE)를 돌릴까요?
