...
| 도구 | 사용 스킬 | 설치 방법 |
| ---- | --------- | --------- |
| `python3 ≥ 3.9` | `ywc-parallel-executor`, `ywc-finish-branch`, `ywc-merge-dependabot` | macOS 12.3+에 기본 설치; `brew install python3` |
| `gh` CLI | Git & Release 스킬 대부분 | `brew install gh` / [cli.github.com](https://cli.github.com) |
...
이 저장소는 Codex 플러그인 배포를 위한 패키징 메타데이터를 [`.codex-plugin/`](.codex-plugin/) 아래에 포함합니다. 플러그인 로컬 skill은 `.codex-plugin/skills/`에 미러링되며 `bash scripts/validate.sh`가 최신 상태를 검사합니다. 이는 Codex CLI/App 플러그인 설치를 준비한 것이며, 공식 Codex 마켓플레이스에 이미 등록되었다는 의미는 아닙니다.

이 저장소를 포함하는 Codex 플러그인 마켓플레이스 또는 소스가 제공되면:
...
인터랙티브 Codex 세션 안에서 플러그인 디렉터리를 열고 `yongwoon/ywc-agent-toolkit`의 **ywc-agent-toolkit**을 검색한 뒤 **Install plugin**을 선택하세요.

### Codex App Plugins 사이드바

Codex App에서는 사이드바의 **Plugins**를 열고 **ywc-agent-toolkit**을 검색하거나 찾은 뒤 플러그인 소스가 `yongwoon/ywc-agent-toolkit`인지 확인하고 플러그인 상세 화면에서 설치하세요.

이 저장소가 사용 가능한 Codex 플러그인 소스에 나타나기 전까지는 아래 bash fallback을 사용하세요.
