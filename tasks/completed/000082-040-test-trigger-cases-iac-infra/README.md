...
`ywc-infra-design`, `ywc-iac-author`, `ywc-infra-review`, `ywc-infra-optimize` (S4, 4개 skill)에 대해 독립적으로 소스된 positive/collision trigger case를 `evals/trigger-cases.json`에 추가하여 S1 coverage floor를 충족시킨다.

## Scope

- `000082-030`이 append한 결과 위에서 이어서 append한다 (Depends On 체인).
...
### Primary Sources
- `docs/ywc-plans/20260812-toolkit-eval-trigger-coverage.md` — FR-3 batch S4 item 목록, Iteration 1–4 amendment 전체
...
### Summary
S4는 IaC/infra lifecycle 4단계로, `ywc-infra-design`(요구사항/설계) → `ywc-iac-author`(Terraform 작성) → `ywc-infra-review`(리뷰) → `ywc-infra-optimize`(사후 최적화) 순서가 서로의 가장 강력한 anti-trigger 후보다.
...
### Depends On
- `000082-030-test-trigger-cases-devenv` — S1+S2+S3 case가 append된 `trigger-cases.json` 상태

### Depended By
- `000082-050-test-trigger-cases-review-quality` — 동일 파일에 이어서 append
...
- dedup: 기존 381개 + `000082-010/020/030`이 append한 case, 그리고 이 task 자신의 output과 대조.
...
### Task Verify
- `python3 .claude/skills/ywc-toolkit-eval/scripts/score.py --target claude-code/skills --item ywc-infra-design --format json` → `signals.coverage.sufficient == true`
- 동일 명령을 `ywc-iac-author`, `ywc-infra-review`, `ywc-infra-optimize`에 대해 반복
- id 중복 없음 확인 (AC7)
