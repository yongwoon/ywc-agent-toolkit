# 000082-030-test-trigger-cases-devenv

## Purpose

`ywc-worktrees`, `ywc-docker-isolate`, `ywc-refactor-clean`, `ywc-onboard-repo` (S3, 4개 skill)에 대해 독립적으로 소스된 positive/collision trigger case를 `evals/trigger-cases.json`에 추가하여 S1 coverage floor(`positives >= 3`, `collisions >= 2`)를 충족시킨다.

## Scope

- `000082-020`이 append한 결과 위에서 이어서 append한다 (Depends On 체인).
- FR-1(mining) → FR-2(fallback authoring) 순서로 이 4개 item 각각에 대해 case를 확보한다.
- Fix C2/S/Z id-inventory, Fix E2/R mining filter, Fix F2 collision override, Fix A2/L/V dedup을 모두 적용한다.

## Spec Reference

### Primary Sources
- `docs/ywc-plans/20260812-toolkit-eval-trigger-coverage.md` — FR-3 batch S3 item 목록, Iteration 1–4 amendment 전체(Operative Sections 우선순위표 기준 최신 Fix 적용)
...
### Summary
S3는 worktree/dev-env 계층으로, `ywc-refactor-clean`이 `ywc-onboard-repo`(신규 저장소 파악)와 자주 헷갈릴 수 있는 후보다. 나머지 sibling은 다른 batch에 있을 수 있어 그쪽도 필요 시 대신 작성한다(Fix I).
...
### Depends On
- `000082-020-test-trigger-cases-spec-execution` — S1+S2 case가 append된 `trigger-cases.json` 상태(dedup 대조 기준선)

### Depended By
- `000082-040-test-trigger-cases-iac-infra` — 동일 파일에 이어서 append

## Key Files
- `.claude/skills/ywc-toolkit-eval/evals/trigger-cases.json` — 4개 item에 대한 새 case를 `cases` 배열에 append
...
- dedup(Fix A2 step 4a + Fix L + Fix V): 기존 381개 + `000082-010`,`000082-020`이 append한 case, 그리고 이 task 자신의 output과 대조. legitimate pair는 예외.
...
### Task Verify
- `python3 .claude/skills/ywc-toolkit-eval/scripts/score.py --target claude-code/skills --item ywc-worktrees --format json` → `signals.coverage.sufficient == true`
- 동일 명령을 `ywc-docker-isolate`, `ywc-refactor-clean`, `ywc-onboard-repo`에 대해 반복
- id 중복 없음 확인 (AC7)
