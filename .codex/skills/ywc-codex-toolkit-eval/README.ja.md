...
この Repository の Codex `ywc-*` Skill と Codex custom Agent の品質を評価するローカル管理 Skill です。`inventory_gate.py` で構造 evidence を集め、`score.py` で mechanical axes を採点し、0-4 rubric の judgment pass で最終 grade と改善 backlog を作ります。

この Skill は配布対象ではありません。`.codex/skills/ywc-codex-toolkit-eval/` の下だけに置き、`codex/skills/` や `.codex-plugin/skills/` には含めません。
...
- `docs/skill-agent-eval/codex/` — Codex 評価 report
