...
### Step 3: SAFE execution loop (dispatch the `ywc-cloud-engineer` persona)

For each SAFE item, in order — reliability-hardening SAFE items flow through this identical loop, checked against `../references/lenses/reliability.md` instead of the cost lens:
...
- **Consults**: `ywc-performance-engineer` persona (cost/utilization signal, Step 1); `ywc-cloud-engineer` persona (SAFE execution worker, Step 3).
...
| Reference | Use when |
|---|---|
| `../references/lenses/cost.md` | Step 1 — cost/utilization signal gathering, right-sizing and waste taxonomy |
...
| `../references/lenses/reliability.md` | Step 2/3 — classification taxonomy for reliability-hardening SAFE/CAUTION candidates (backups, health checks, autoscaling floors, deletion protection, multi-AZ) and the guardrail against a cost change dropping below the reliability floor |
