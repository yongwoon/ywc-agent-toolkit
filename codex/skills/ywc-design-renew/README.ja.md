...
デザインへ renew し、同時に AI-slop tell を点検する Codex Skill です。
...
- (任意) `impeccable` skill — あれば強力な design engine として委譲、無ければ自前
  ruleset に fallback。Project が対応していれば `npx impeccable skills install` で外部
  skill を install できます。install 後に `impeccable init` を 1 回実行すると project の
  Design Context が設定され、下記の `PRODUCT.md` / `DESIGN.md` が生成されて再質問が省略されます。
