...
6. **Excerpt extraction (`source: "plan"` or override only).** Bound the read before it happens — a large plan file would otherwise burn the context this PR still needs. Read at most the first 120 lines of the matched file, never the whole file. Within that window, take the content under `## Goal` or `## Purpose` until the next `##` heading, capped at 5 lines / approximately 500 characters. If neither heading exists, use the first paragraph after the title with the same cap. Never forward the full file. Skip this sub-step for `source: "task"`; use its extracted summary verbatim.
