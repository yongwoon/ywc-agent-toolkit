...
ywc-task-generator Skill で生成された Task を実行する Codex Skill です。Branch 作成から実装、Commit、PR 作成、CI 確認、Merge、Local sync まで、開発 Lifecycle 全体を自動化します。
...
Range 全体を 1 つの work branch に蓄積し、最後に PR 1 つで delivery します:
...
| `--aggregate-pr` | Range を 1 つの work branch に蓄積し、final work -> base PR 1 つで delivery | `--aggregate-pr` |
| `--group-name <name>` | Aggregate work branch 名を指定 (`work/<name>`)。`--aggregate-pr` のみ有効 | `--group-name billing-rollout` |
...
> `--group-name` は `--aggregate-pr` なしでは使用できません。
...
```text
work/<name> 作成 → Task ごとに work branch から feature branch 作成 → work branch へ local merge → 最後に work -> base PR 1 つを作成/ready/CI/bot/merge
```
...
Step 1: Dependency Validation & Spec Loading
  ...
  └─ 外部 URL は Codex approval settings or the project-local policy file の taskExecutor.externalSpecUrls policy に従う
...
Step 2: Branch Creation (毎 Task 実行 — range でもスキップしない)
  ...
  └─ (--aggregate-pr) work branch から feature branch 作成
...
Step 5: PR Creation
  ...
  └─ (--aggregate-pr) 各 Task は work branch へ local merge
...
Final Aggregate PR (--aggregate-pr only)
  └─ 最後の Task 後に work -> base PR 作成、ready、CI、bot review、merge-readiness、merge、local base sync
```
...
### Aggregate PR による group delivery
...
`--local-merge`, `--draft`, `--skip-ci-wait`, `--aggregate-pr` を同時に指定すると、Skill は実行を中断し、どの mode を意図したか確認します。この 4 つは異なる終了状態を作るためです。
