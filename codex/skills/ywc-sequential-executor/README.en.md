...
- `--resume-disposition resume|stop`: supplies the checkpoint decision without opening a prompt; missing or mismatched context returns bounded `NEEDS_CONTEXT`.
...
Non-interactive transitions never wait for progress or approval. Resume, branch/worktree conflict, CI wait/timeout, and external URL policy branches return bounded terminal statuses. Handoff is written atomically after checkpoint/task state is authoritative and never changes checkpoint, completion, cleanup, or deletion state.
