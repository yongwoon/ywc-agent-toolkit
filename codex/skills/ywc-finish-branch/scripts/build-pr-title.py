...
def parse_task_name(task_name: str) -> tuple[str, str]:
    ...
    # New format: [initials-]000001-010-<slug>. Requiring a letter in the
    # optional namespace preserves the established numeric N-M fallback.
    m = re.match(r'^((?=[a-z0-9]{2,4}-\d{6})(?=[a-z0-9]*[a-z])[a-z0-9]{2,4}-\d{6}-\d{3})-(.+)$', task_name)
    if m:
        return m.group(1), m.group(2)
    # Legacy format: [initials-]001010-<slug>
    m = re.match(r'^((?=[a-z0-9]{2,4}-\d{6})(?=[a-z0-9]*[a-z])[a-z0-9]{2,4}-\d{6})-(.+)$', task_name)
    if m:
        return m.group(1), m.group(2)
