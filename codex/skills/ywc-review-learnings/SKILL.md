...
| `--source` | `--source feedback\|review\|pr\|mining` | `feedback` | Where a new learning comes from in `update` mode (see Capture Sources) |
...
6. **Codex project-instructions prompt.** The *first* time the file is created (it did not exist before this run), print the activation reminder so the learnings actually reach future reviews:
   ```text
   To activate these learnings for future Codex sessions, add a short instruction
   to AGENTS.md or CODEX.md telling agents to read docs/review-learnings.md before
...
The `--source mining` handoff accepts only a complete, user-confirmed changeset
from `ywc-mine-review-history`: each cluster must include its generalized rule,
why, polarity, target scope, representative evidence, and a list of distinct PR
numbers supporting the recurrence. The miner is a producer and has no durable
write authority; this skill remains the sole owner of project-local writes.
...
- [ ] On first file creation, the AGENTS.md / CODEX.md integration prompt was printed
...
- **Downstream**: `ywc-impl-review` consumes the read-mode Applicable Learnings block; project instructions can also tell Codex sessions to read `docs/review-learnings.md` before implementation reviews
