...
## Worktree Granularity

`ywc-parallel-executor` uses one worktree per task because tasks in the same
wave run concurrently and must keep branches, Docker state, and recovery state
isolated. This differs from `ywc-sequential-executor --worktree`, which uses one
run-level worktree for the entire sequential invocation because only one task is
active at a time. Do not reuse the sequential run-level worktree model here; it
would collapse wave isolation and make per-task cleanup/recovery ambiguous.
