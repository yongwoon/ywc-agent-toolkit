...
> Worktree lifecycle is owned by the dedicated `ywc-worktrees` skill (priority resolution + `--mode create` / `audit` / `prune` / `resolve`). This skill delegates worktree creation, audit, and cleanup to `ywc-worktrees` instead of inlining the procedures. See `ywc-worktrees` for the four-mode contract and the priority chain (`.worktrees/` > CLAUDE.md `worktree_root` > caller-specified > legacy fallback). The bundled `audit-worktrees.sh` / `cleanup-worktree.sh` scripts have moved to that skill.
...
```
The `--waves` array is the wave plan from Step 3 — one entry per wave, `tasks` listing that wave's task-directory names, `has_contract` from the scan above. See the schema in [references/checkpoint-resume.md](references/checkpoint-resume.md).
...
- **Mobile-first UI directive (append to applicable frontend-worker prompts):** Read [the mobile-first policy](../references/mobile-first-ui.md) before new or redesigned end-user UI layout/style work; author narrowest-viewport base first, then expand with `min-width`. Skip only for an explicit PC/tablet-only exception; do not retrofit unchanged legacy desktop-first UI or apply this to admin/internal or non-UI work.
...
**Handling each subagent's status return**: immediately after each successful dispatch and before routing, register the source task name against its returned canonical target and apply [`subagent-async-monitoring.md`](../references/subagent-async-monitoring.md) — only a task with a confirmed terminal status proceeds to Status Routing; a missing identity or possibly-live target is a non-terminal blocker for cleanup, next-wave dispatch, reports, and `DONE`. Each subagent then ends with `DONE`, `DONE_WITH_CONCERNS`, `BLOCKED`, or `NEEDS_CONTEXT`. The orchestrator's response is defined by [../references/subagent-status-actions.md](../references/subagent-status-actions.md). In particular: `NEEDS_CONTEXT` → provide the missing context and re-dispatch the same subagent at the same model class (context is the cheapest fix); `BLOCKED` → run the four-step triage (context → reasoning → scope → plan) before surfacing to the user; `DONE_WITH_CONCERNS` → read the concerns and decide whether they are correctness-level (fix and re-dispatch) or observation-level (carry forward to the Completion Report). Do not silently retry the same subagent on the same input — change the input or the model class between attempts.

**Status Routing** — Apply to every per-task subagent return inside the wave loop, once async monitoring above has confirmed a terminal status:
...
| Status absent or unparseable, or no completion notification within the escalation threshold | Run the four-step triage, preserve the task branch/worktree if unresolved, and apply Wave Failure Handling. |
...
**4c.5. Per-task quality gate (optional contract)** — After 4c passes and before any task can enter Wave Delivery, resolve the task's conditional Quality Gate Contract using [../references/quality-gates.md](../references/quality-gates.md). A task carrying exactly `N/A — no quality gate contract` keeps the existing path: emit no placeholder packet, select neither worker, and do not require a gate tool. `report-only` likewise records only sanitized evidence or an unavailable-tool gap and selects neither write-enabled worker. For `advisory` or `enforced`, reject an incomplete packet, mismatched command identity/digest, invalid evidence destination, or missing production/test Ownership with `NEEDS_CONTEXT` before dispatch; never infer a command, threshold, path, symbol, or pass. For each eligible task, dispatch exactly one Cleaner attempt, keyed by task identity, only when sanitized complexity evidence exceeds the caller-approved threshold. The Cleaner receives the unchanged packet and may edit only `ownership.production_paths` and `production_symbols`; it must return sanitized status, pre/post baseline evidence, measured CRAP, and changed paths. It has no staging, commit, push, PR, merge, delivery, or delegation authority. At or below threshold, record the sanitized passing evidence and mark Cleaner `skipped — threshold met`; this skip is not a worker result. A Cleaner `BLOCKED` or `NEEDS_CONTEXT` is disallowed: preserve that task's worktree and branch, do not authorize Hardener for it, and route it through the existing failure handling. A permitted Cleaner `DONE` or `DONE_WITH_CONCERNS` remains attributable to that task and is carried forward without being overwritten. Do not silently retry a Cleaner with the same dispatch identity: recovery must change bounded context or model class, remain within the one-Cleaner idempotency guard, and stop after two failed attempts as `BLOCKED` while preserving the worktree and branch. Do not copy raw command text, output, transcripts, secrets, or full diffs into worker payloads, handoffs, or reports.
**4c.6. Ownership rejection and retry guard** — A request for a production path or symbol outside `ownership.production_paths`/`production_symbols`, or a test/fixture path or symbol outside the Hardener Ownership, is rejected as `NEEDS_CONTEXT` before any worker dispatch or edit; do not broaden the packet. Cleaner and Hardener retries are keyed by task identity plus gate kind (and wave for Hardener): recovery may change bounded context or model class, but must not silently repeat the same dispatch identity. After two failed recoveries from a recoverable tool error, preserve the task worktree and branch and record `BLOCKED`.
...
**4e. Wave Delivery + Mark Complete** — After all tasks in the wave complete their implementation, deliver successful tasks sequentially and mark each one complete. Delivery is required for **every** mode because downstream waves branch from the base. For `--local-merge`, `--draft`, and `--aggregate-pr`, a contract-bearing wave delivers onto `wave-int/<N>` instead of directly onto base — see [references/wave-integration-branch.md](references/wave-integration-branch.md); post-promotion base content is identical to the integration branch, so "the base branch" below still means the branch the next wave assumes when it starts. A fully contract-less wave, or any wave under `--per-task-pr`, delivers direct to base exactly as today. For `--local-merge`, `--draft`, and `--aggregate-pr`, the per-task `git merge --no-ff` + post-merge verification + Mark Task Complete + push (or defer) is delegated to `ywc-finish-branch`. For `--per-task-pr`, this skill runs the full PR lifecycle (create → CI → bot → `gh pr merge` → base sync) and an inline Mark Complete, because the worktree model is incompatible with finish-branch's `normal-pr` mode (which assumes the feature branch is the current checkout and runs a local `git checkout <base>` that cannot execute from a worktree).
...
**4e.5. Wave-boundary Hardener and aggregation** — After every task in the wave has completed 4c.5, collect only sanitized mutation evidence at this boundary, dispatching at most one Hardener attempt per eligible task (keyed by task identity and wave, edits only `ownership.test_fixture_paths`/`test_fixture_symbols`, canonical attempt cap three, survivors reportable after the cap) exactly as today. Before delivery — meaning before the promotion step below, now that delivery to base for a contract-bearing wave happens in two phases (per-task merge onto `wave-int/<N>`, then promotion) — aggregate all task and gate results at the wave boundary with fixed precedence `BLOCKED > NEEDS_CONTEXT > DONE_WITH_CONCERNS > DONE`; under `advisory` an unavailable tool/capped survivor is `DONE_WITH_CONCERNS`, under `enforced` it is `BLOCKED`, under `report-only` it is recorded without mutation. **The carve-out rule** — an isolation branch can only protect state whose point-of-no-return sits after the gate — means the aggregate keeps reporting authority only (no stage/commit/push/PR/merge/delivery) for `--per-task-pr` (its point-of-no-return, `gh pr merge --delete-branch`, sits inside the wave per task; blocking authority stays at 4c.5), but for `--local-merge`/`--draft`/`--aggregate-pr` on a contract-bearing wave, an `enforced`-tier `BLOCKED` outcome now additionally **gates** 4e.6 below. See [references/wave-integration-branch.md](references/wave-integration-branch.md) for the full rationale and dispatch-eligibility details unchanged from today. **Checkpoint** (contract-bearing wave, three isolated modes): immediately after, run `python3 "$STATE_SCRIPT" hardener-verdict <N> <absent|PASS|BLOCKED|NEEDS_CONTEXT> [--detail <text>]` (verdict-mapping rule in the same reference) — `BLOCKED` and `NEEDS_CONTEXT` are written **only** when the aggregate outcome arose under an `enforced` contract; a `report-only`/`advisory`-tier NEEDS_CONTEXT-equivalent structural dispatch failure (per `quality-gates.md:97`, "regardless of enforcement mode") is recorded in the Completion Report only and must not be written as the `hardener_verdict` literal, since 4g/4i below key off this field's literal value without a separate tier check. When the verdict is `NEEDS_CONTEXT`, always pass `--detail` with Hardener's own missing-context diagnostic; this writes the dedicated `hardener_detail` checkpoint field, distinct from `blocked_detail`, so a later resume can display the actual gap. Store the diagnostic in a shell variable first and pass it double-quoted (`--detail "$hardener_detail"`) — never interpolate Hardener's free-form text directly into the command line, since unquoted whitespace causes argument splitting and an unquoted `$()`/backtick sequence in the diagnostic would be evaluated as command substitution. 4e.6 below reads this value.
**4e.6. Promotion (gated on the wave-boundary aggregate, `--local-merge`/`--draft`/`--aggregate-pr` only)** — For a wave that created `wave-int/<N>` (i.e. its `integration_branch` is non-`None`), promote it into base only when the aggregate outcome above is neither an `enforced`-tier `BLOCKED` nor `NEEDS_CONTEXT`; `wave-complete` is stamped only after a successful promotion. A fully contract-less wave, or any `--per-task-pr` wave, already delivered direct to base at 4e and is unaffected. Fast-forward failure, promotion-conflict handling, and the blocked-wave preservation behavior are in [references/wave-integration-branch.md](references/wave-integration-branch.md) — do not reimplement the sequence here.
**Checkpoint**: use the `STATE_SCRIPT` resolver from State Init, then run `python3 "$STATE_SCRIPT" wave-complete <N>` — flips wave `<N>` to `completed` (it refuses if any task is still `pending`, a built-in guard against marking an incomplete wave done) and stamps `last_checkpoint`. For a contract-less wave (or any `--per-task-pr` wave), run this immediately after the wave loop finishes, unchanged from today. For a contract-bearing wave under `--local-merge`/`--draft`/`--aggregate-pr`, run this only after 4e.6's promotion succeeds — never immediately after the per-task loop, since the wave is not yet delivered to base until promotion lands.
...
For each task whose Step 4e delivery completed (DONE) **and whose wave is not in a blocking-verdict wave (`Hardener-BLOCKED` or `Hardener-NEEDS_CONTEXT`)** (i.e. the wave created no `wave-int/<N>`, or its `hardener_verdict` is not in {BLOCKED, NEEDS_CONTEXT} — check `.ywc-run-state.json`; a task whose wave is `Hardener-BLOCKED` or `Hardener-NEEDS_CONTEXT` skips this step per [references/wave-integration-branch.md](references/wave-integration-branch.md)):
...
- `Hardener-BLOCKED` / `Hardener-NEEDS_CONTEXT` wave (third bucket, `--local-merge`/`--draft`/`--aggregate-pr` only) — every task in the wave individually succeeded (`DONE`, already under `completed/`) but the wave's `wave-int/<N>` promotion at 4e.6 has `hardener_verdict in {"BLOCKED", "NEEDS_CONTEXT"}`. Task-directory location alone cannot distinguish this from a fully completed wave, since Mark Complete already ran before the wave-boundary gate — key off `.ywc-run-state.json`'s `status`/`hardener_verdict`/`integration_branch` instead. See [references/wave-integration-branch.md](references/wave-integration-branch.md).
