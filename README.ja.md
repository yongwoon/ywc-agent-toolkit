...
現在、Claude Code skill 42 個、Codex skill 52 個、Claude Code agent 12 個、Codex custom agent 8 個を提供しています。
...
Resolution は explicit `--lang` > project `.codex/ywc.json` > `AGENTS.md` /
`CODEX.md` / `CLAUDE.md` > user `~/.codex/ywc.json` > user に質問、の順です。
Session default はサポートしません。
...
Codex には `ywc-*` skill を補完する **8 個**の read-only specialist agent が含まれます。`~/.codex/agents/` にインストールされます。
