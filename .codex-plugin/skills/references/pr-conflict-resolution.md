...
the entry point differs (after PR creation, after review fixes, before merge, or
per-task in a wave).
...
often `UNKNOWN` for a few seconds. Poll in a **single shell call** using the
`until` pattern instead of a detached `sleep` followed by a second command:

```bash
# Run this ENTIRE block as a SINGLE shell call.
MERGEABLE=UNKNOWN
TRIES=0
...
the merge gate is only satisfied when `mergeable == MERGEABLE` **and** CI is
green on the new head SHA.
...
2. Surface to the user: the PR URL, the base branch, the conflicting file list,
   and a one-line description of why the base advanced (for example, "a sibling
   task merged into `<base>` first").
