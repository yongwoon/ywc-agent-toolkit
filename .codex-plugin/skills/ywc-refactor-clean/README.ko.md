# ywc-refactor-clean (한국어)

Detection tool (knip / depcheck / ts-prune / vulture / deadcode / cargo-udeps) 기반 dead code 제거 Skill 입니다. Finding 을 SAFE / CAUTION / DANGER 3-tier 로 분류한 뒤 per-item 단위로 Test 를 돌리고 commit 합니다. Behavior 변경 (duplicate consolidation 중 semantics 재조정 등) 은 본 Skill 범위 밖이며 `ywc-tdd-ritual` + `ywc-code-gen` 으로 routing 합니다.
