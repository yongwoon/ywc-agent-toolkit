...
| `ywc-skill-author/scripts/enumerate-rd-rows.sh <skill-dir> \| --self-check` | ywc-skill-author | Rationalization Defense table data-row `<start>-<end>` line-range enumeration, mirroring score.py's canonical row counter; `--self-check` asserts 46/46 count parity |
| `ywc-skill-author/scripts/build-variant.sh <skill-dir> <start> <end>` | ywc-skill-author | Writes a temp-path variant of SKILL.md with the given inclusive line range deleted; exit 1 (no write) on inverted/out-of-bounds range or header orphan |
...
| `ywc-handle-pr-reviews/scripts/fetch-nitpick-comments.sh <owner/repo> <pr>` | ywc-handle-pr-reviews | Nitpick-tier fetch + dedup-by-hash + addressed-marker exclusion; exit 0 = JSON array |
