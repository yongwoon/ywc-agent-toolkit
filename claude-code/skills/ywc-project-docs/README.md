...
언어는 자동 추정하지 않습니다. `--lang kr|ja` 옵션이 있으면 그대로 사용하고, 없으면 한국어/일본어 중 어느 언어로 작성할지 먼저 질문합니다.
...
1. **언어 선택** — `--lang` 옵션 우선, 없으면 한국어/일본어를 질문하여 결정 (자동 추정 안 함)
...
```
~/.claude/skills/ywc-project-docs/
├── README.md     ← 이 파일 (한국어)
