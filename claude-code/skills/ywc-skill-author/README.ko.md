...
새로운 ywc-* skill을 작성하거나 기존 skill의 구조를 개선할 때 사용하는 **메타 skill**입니다. 46개 production ywc-* skill에서 도출된 표준 규칙(Frontmatter format, Rationalization Defense, multilingual triggers, progressive disclosure 등)을 LLM이 자동으로 따르도록 강제합니다.

## 사용 시나리오

- 새 ywc-* skill을 처음부터 작성할 때
- 기존 ywc-* skill의 frontmatter, body section, references 구조를 개선할 때
- 46개 ywc-* skill을 표준 rule set 기준으로 audit할 때
...
상세한 내용은 `SKILL.md` 본문과 `references/` 하위 6개 문서를 참조합니다.
...
- `references/cross-skill-graph.md` — 46개 ywc-* skill 간 의존성 + cross-reference 그래프
- `references/progressive-disclosure.md` — Tier 2 vs Tier 3 배치 결정 트리 (A14 규칙)
- `references/audit-workflow.md` — read-only `--audit` workflow의 계약 및 역할 매트릭스
