...
4. **Phase 4 — Implementation** — Regression test → 単一 fix → red-green-red 検証 → `ywc-verify-done` で gating → §6 systemic prevention emit (recurring class なら `ywc-review-learnings --source debug` で promote を提案、そうでなければ one-off を明示)
