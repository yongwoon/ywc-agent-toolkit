...
| "The subagent's return payload says success, that's enough" | Subagent self-reports are the **second-most** common false-positive after "I think it works". Always check the VCS diff or run the verification yourself per `references/subagent-status-actions.md`. A `Write`-capable subagent's canonical payload carries only an artifact path, not the content — the work itself is on disk and must be verified. (A read-only agent under the §3.5 exception has no disk output to verify; its inline findings are verified by re-reading the same evidence it cited.) |
