...
`--aggregate-pr` is `--draft` plus the merge tail. The per-wave accumulation (the Step 4e
`--defer-push` local merges onto the local base) is identical to `--draft`; `--aggregate-pr`
diverges only at the **end-of-run** step — instead of Section A's draft branch + draft PR, it
follows B1–B7 below to carve `aggregate/<name>`, open a **ready** PR, and run the full
`ready → CI → bot → merge → base sync` lifecycle. Steps:

### B1. Create the aggregate branch

Derive the branch name from `--group-name` when provided (preferred — it makes concurrent
groups distinguishable), otherwise fall back to a timestamp:
...
or `BEHIND` if the base advanced (e.g. a sibling group merged first). Read
...
- `BEHIND` / merely out-of-date → update the branch by **merging base into the branch**
  ...
  git checkout <base-branch>
  ```
- A real textual `CONFLICTING` state is a **1-attempt, surface-to-user `BLOCKED`** situation
  — never auto-resolve, never force-push, never `git merge --abort` into a discard.
...
**at the same time**, you must isolate each group's **local base branch**, because the
...
| `.git` refs and branches (incl. the local `<base>` branch) | **shared** | `git reset --hard origin/<base>` + per-task merges mutate the **shared** local `<base>` ref — groups corrupt each other |
| Branch checkout exclusivity | shared | `<base>` can be checked out in only one worktree (`fatal: '<base>' is already checked out`) — the second group cannot even start its accumulation |
...
> worktree-safe (distinct branches can live in distinct worktrees). The current mode reuses
> the `--draft` base accumulation, so that isolation is not yet available; use separate
> clones.
...
Combine with `--review` to gate each task's worktree branch through `/ywc-impl-review`
before it joins the aggregate, and `--pr-lang` to fix the final PR language.
