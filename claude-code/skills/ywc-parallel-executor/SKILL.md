...
**4b-monitor (mechanical gate — do not skip)**: before treating the wave as complete, the orchestrator must reconcile the full expected roster (every task name dispatched in this wave) against the set of task names it has received a terminal status for, per the Unique Dispatch Labeling and Full-Roster Reconciliation rules in [../references/subagent-async-monitoring.md](../references/subagent-async-monitoring.md) — never treat the wave complete without one terminal status per dispatched task name. Illustrative check: `for t in <wave-task-names>; do grep -q "$t" "$RECONCILED_SET" || echo "missing: $t"; done`.
