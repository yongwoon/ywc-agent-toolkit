...
description: >-
  ...
  IAM, public exposure, secrets in state, cost right-sizing, and
  SPOF/backup gaps across AWS/GCP/Azure/K8s Terraform. Fans out to
  security/cost/reliability lenses with severity-rated findings. Triggers:
  "인프라 리뷰", "terraform 검토", "iam 과권한", "보안 그룹 점검", "infra review",
  "review my terraform", "インフラレビュー", "ywc-infra-review". Do not use for app-code
...
Dispatch `Task(subagent_type: ywc-security-engineer)` carrying the scoped Terraform files and an explicit pointer to [`../references/lenses/security.md`](../references/lenses/security.md) and [`../references/iac-security.md`](../references/iac-security.md). This lens covers: security groups / firewall rules, IAM / RBAC over-permission, public exposure (buckets, endpoints, load balancers), and secrets committed to code or state.

### Step 3: Cost lens

Dispatch `Task(subagent_type: ywc-performance-engineer)` carrying the scoped Terraform files and an explicit pointer to [`../references/lenses/cost.md`](../references/lenses/cost.md) and [`../references/finops.md`](../references/finops.md). This lens covers: compute right-sizing against observed/expected utilization, idle or orphaned resources, and pricing model fit (on-demand vs reserved vs spot).

### Step 4: Reliability lens

Dispatch `Task(subagent_type: ywc-cloud-engineer)` in **review mode** — carry the scoped Terraform files and an explicit instruction that this dispatch is read-only diagnosis: it authors nothing, runs no `terraform apply`, and confines itself to the reliability lens at [`../references/lenses/reliability.md`](../references/lenses/reliability.md). This lens covers: single points of failure, missing backups / point-in-time recovery, missing resource limits or autoscaling floors, and missing health/readiness checks.
...
- [ ] The reliability lens dispatch to `ywc-cloud-engineer` was explicitly instructed as review-only (no authoring, no `apply`)
...
- **Confusing this skill with `ywc-security-audit`.** That skill reviews application-code auth/injection; this skill's security lens reviews IaC misconfiguration — different taxonomy, different agent dispatch.
...
- **Consults**: `ywc-security-engineer` (security lens), `ywc-performance-engineer` (cost lens), `ywc-cloud-engineer` in review mode (reliability lens).
...
| Reference | Use when |
|---|---|
| [`../references/lenses/security.md`](../references/lenses/security.md) | Step 2 — security lens dispatch, IaC misconfiguration taxonomy |
| [`../references/iac-security.md`](../references/iac-security.md) | Step 2 — the detailed IaC misconfiguration taxonomy `ywc-security-engineer` reviews against |
| [`../references/lenses/cost.md`](../references/lenses/cost.md) | Step 3 — cost lens dispatch, right-sizing and waste taxonomy |
| [`../references/finops.md`](../references/finops.md) | Step 3 — the detailed FinOps taxonomy (reserved/spot, data-transfer cost) `ywc-performance-engineer` reviews against |
| [`../references/lenses/reliability.md`](../references/lenses/reliability.md) | Step 4 — reliability lens dispatch, SPOF/backup/health taxonomy |
| [`../references/providers/aws.md`](../references/providers/aws.md) | Scoped IaC targets AWS — provider-specific misconfiguration/cost/reliability signals |
| [`../references/providers/gcp.md`](../references/providers/gcp.md) | Scoped IaC targets GCP — provider-specific misconfiguration/cost/reliability signals |
| [`../references/providers/azure.md`](../references/providers/azure.md) | Scoped IaC targets Azure — provider-specific misconfiguration/cost/reliability signals |
| [`../references/providers/k8s.md`](../references/providers/k8s.md) | Scoped IaC includes Kubernetes/Helm (via Terraform providers) |
| [`../references/subagent-status-actions.md`](../references/subagent-status-actions.md) | Steps 2–4 — the §3.5 return-payload contract each lens dispatch must inject so three fan-out returns stay bounded |
