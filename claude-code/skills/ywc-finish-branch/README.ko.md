...
- **Worktree 생애주기 미관여, 단 worktree 안 동작은 지원**: worktree 생성/제거는 caller 책임이나, `--worktree-path <path>` 를 주면 Steps 1·5–8 의 모든 git 명령을 `git -C <path>` 로 수행하여 `ywc-sequential-executor --worktree` 가 만든 run worktree 안에서 delivery 가능 (delivery 의미 불변, working dir 만 변경)
