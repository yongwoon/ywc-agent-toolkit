...
기존 / 처음 보는 Repository 진입 시 4-Phase (Reconnaissance → Architecture → Conventions → Generate) 로 Onboarding Guide 와 Starter CLAUDE.md 를 생성하는 Skill 입니다. 신규 Repository 생성 (scaffolding) 이 아니며, `ywc-project-scaffold` 의 inverse direction 입니다. Reconnaissance 는 Glob + Grep 만 사용하여 token cost 를 통제합니다.
...
- **Output B**: Starter CLAUDE.md (repo root 에 write) — 기존 file 있으면 `## Detected Conventions (<YYYY-MM-DD>)` section 만 추가. 기존 `AGENTS.md` (vendor-neutral 표준) / `.cursorrules` 가 있으면 함께 Read 하여 reconcile — 생성한 CLAUDE.md 가 그것과 모순되지 않게 함 (AGENTS.md emit 은 Codex variant 담당, 의도적 divergence)
