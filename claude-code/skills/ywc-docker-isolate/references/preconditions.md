...
## Port-var source 우선순위 (FR-2.1 / Q2)

1. `--port-vars VAR1,VAR2` (명시 인자) — 최우선.
2. CLAUDE.md `docker_port_vars:` directive — 존재 시 auto-detect override.
3. compose `${VAR:-NNNN}` short-syntax auto-detect — 기본.

> ⚠️ **override 시 hardcoded-port(AC5) 검사 생략:** `--port-vars` 또는 CLAUDE.md
> directive 로 VAR 을 명시하면 auto-detect 의 hardcoded-port fail-loud(AC5) 분기를
>거치지 않는다 — 호출자가 격리 대상을 책임진다. 명시한 VAR 외에 compose 에
> hardcode 된 port 가 남아 있으면 그 port 는 격리되지 않으니 주의한다.
...
| long-syntax | `published: "${APP_PORT:-3000}"` | ❌ 미탐지 — `--port-vars` / CLAUDE.md 로 명시 |
