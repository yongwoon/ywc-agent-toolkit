...
# docs/ywc-plans/ywc-docker-isolate.md (Operative Sections authoritative).
#
# Usage:
...
# Priority (Q2): CLAUDE.md docker_port_vars override > compose auto-detect.
detect_claude_override() {
  local f
  for f in "$WORKTREE_PATH/CLAUDE.md" "CLAUDE.md"; do
    [[ -f "$f" ]] || continue
    grep -E '^[[:space:]]*docker_port_vars:' "$f" 2>/dev/null | head -n1 \
      | sed -E 's/^[[:space:]]*docker_port_vars:[[:space:]]*//; s/[]["'"'"']//g; s/[[:space:]]+/,/g' \
      | tr ',' '\n' | grep -vE '^$' && return 0
  done
  return 1
}
...
if [[ -n "$PORT_VARS_OVERRIDE" ]]; then
  ...
  CLAUDE_VARS=$(detect_claude_override || true)
  if [[ -n "$CLAUDE_VARS" ]]; then
    while IFS= read -r v; do [[ -n "$v" ]] && TARGET_VARS+=("$v"); done <<< "$CLAUDE_VARS"
  else
    ...
    if [[ ${#TARGET_VARS[@]} -eq 0 ]]; then
      if [[ "${HAS_PORTS:-0}" == "1" ]]; then
        ...
        die "ports hardcoded — cannot isolate (use short-syntax \${VAR:-NNNN} or CLAUDE.md docker_port_vars)"
      fi
