...
- Unlike [Go Large (DDD)](#go-large-ddd) above, domains are not split into separate top-level packages — each layer package (`domain/`, `usecase/`, `repository/`, `handler/`) holds every domain's files, grouped by sub-purpose (`entity/`, `converter/`, `port/`)
...
- Choose this over [Go Large (DDD)](#go-large-ddd) when the service is one deployable with many CRUD-shaped domains sharing infrastructure; choose Bounded Context DDD when domains genuinely diverge and may split into separate services later
...
| `injector/`        | DI container wiring (dig/wire provider sets), wired at startup — not needed for Go Small/Medium |
| `gen/`             | Generated protobuf/Connect-RPC stubs only — never hand-edit. DB-layer codegen (if used) belongs under its own package (e.g. `infrastructure/db/gen/`), not top-level `gen/` |
| `converter/`       | Dedicated entity↔DTO / entity↔DB-model mapping subdirectory, kept out of business logic |
