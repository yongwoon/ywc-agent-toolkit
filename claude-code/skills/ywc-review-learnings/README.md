...
- LLM이 Project별 review 선호를 자동으로 이해하도록 CLAUDE.md에 `@docs/review-learnings.md` 참조를 추가하고 싶을 때
...
- (선택) `--source feedback|review|pr|debug|incident` — update 시 learning 출처 (기본 `feedback`; `debug`/`incident` 는 root-cause / incident prevention 항목 포착)
...
- 최초 파일 생성 시: CLAUDE.md 에 `@docs/review-learnings.md` 추가를 권장하는 활성화 안내 출력 (이 참조를 더해야 이후 모든 review 와 LLM session 이 learning 을 자동으로 불러옵니다)
