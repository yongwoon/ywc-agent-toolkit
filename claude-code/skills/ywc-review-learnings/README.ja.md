...
- LLM が Project ごとの review の好みを自動的に理解するよう、CLAUDE.md に `@docs/review-learnings.md` 参照を追加したいとき
...
- (任意) `--source feedback|review|pr|debug|incident` — update 時の learning の出所（既定 `feedback`；`debug`/`incident` は root-cause / incident prevention 項目を capture）
...
- ファイル初回作成時: CLAUDE.md に `@docs/review-learnings.md` の追加を勧める activation 案内を出力（この参照を加えることで以降の全 review と LLM session が learning を自動的に読み込みます）
