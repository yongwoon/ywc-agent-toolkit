...
| "Phase 1 already marked this Critical, no need to double-check" | Phase 1 "Confirmed" is a single lane's self-assessment, not independent confirmation — the same failure mode already named for "reviewer agents agree, so the finding is correct." Skipping Step 4.5 on a Critical/High Confirmed finding (including a Security finding confirmed at Opus) reintroduces exactly the false-Confirmed-positive risk it exists to catch. |
...
**Boundary with Step 4.5**: the Independent Verification Pass (Step 4.5) is a separate mechanism — an independent Sonnet reproduction check on Critical/High Confirmed findings from any Phase 1 aspect (including Security's Opus-sourced Confirmed findings), not a Sonnet→Opus escalation — and is not governed by this section's budget or Pattern B.
...
2.5. **Scope Guardrail** — Before dispatching Phase 1, check the scope resolved in Step 2:
   - **Empty scope**: this restates Step 2's existing `NEEDS_CONTEXT` rule for emphasis before the excessive-scope check below runs, rather than introducing a second, conflicting rule — if the `--git-range` changed-file list is empty, the `--code` path resolves to zero reviewable files, or `--working-tree` finds no staged/unstaged/untracked changes, stop here per Step 2's rule: return `NEEDS_CONTEXT` and report "Nothing to review: no changed files found for `<--git-range value, --code path, or working tree>`", suggesting the user check staging/commit state or the `--git-range`/`--code` value. Do not dispatch Phase 1.
   - **Excessive scope (hard block)**: for `--git-range` and `--working-tree` — both have a real diff available (a commit range, or the combined staged/unstaged/untracked diff) — refuse to dispatch Phase 1 when the changed-file count exceeds **200 files** or the total changed-line count (added + removed) exceeds **5,000 lines**. For `--code`, apply the 200-file limit only — do not invent a changed-line count when no diff is available for a raw path target. Report the exact file count, line count (when applicable), and the top 5 files by changed-line count, then instruct the user to narrow `--git-range` to a smaller commit range, reduce the working-tree change set, or split `--code` into multiple invocations. These thresholds are deliberately tighter than the cloud `/code-review ultra` feature's 500-file/8,000-line limits: each of this skill's five Phase 1 subagents independently Reads full file contents per Step 3's bounded-dispatch model, so a large diff multiplies local session cost five-fold rather than once. A diff this large on a task-sized branch — this skill's typical entry point via `ywc-sequential-executor`/`ywc-parallel-executor`'s `--review` flag — usually signals an upstream task-decomposition problem, not something to silently absorb here.

3. **Phase 1 — Parallel Executor Review** — Use the Task tool to spawn five subagents in parallel. Pass `model` explicitly on each call — Architecture / Design / Devex / QA stay at Sonnet or Haiku cost; Security runs at Opus. Apply [../references/subagent-async-monitoring.md](../references/subagent-async-monitoring.md) for roster reconciliation across this 5-way concurrent dispatch:
   ...
   - **QA subagent** (`model: haiku`) — Test coverage gaps and missing test cases. Reference: `references/qa-agent.md`. Haiku is appropriate because coverage-gap detection is largely mechanical (file enumeration, assertion counting, branch enumeration) and does not typically require frontier reasoning. When the task's Quality Gate Contract ran a Mutation gate, a surviving mutant is required evidence for a P1 Testing finding; absence of a contract or a 0-survivor result must not itself be read as license for unsupported subjective Testing criticism (see `gate_state` in [../references/quality-gates.md](../references/quality-gates.md)), but must also not force additional criticism where none is warranted.
   ...
   **Inject the Return-payload contract directive** into every one of the five subagent prompts, quoted **verbatim** from [`../references/subagent-status-actions.md`](../references/subagent-status-actions.md) §Return Payload Contract:
   ...
   The Architecture, Design, Devex, and QA subagents must each produce two categories of output — **Confirmed findings** and **Advisor candidates** (findings flagged for Phase 2 review; each candidate must include the finding text, a bounded code snippet (≤100 lines), the relevant spec excerpt (if any), and a one-sentence reason the executor wants a second opinion). A Write-capable subagent (Design, Devex, QA, or the default generic Architecture dispatch) produces both **written to a file with only the path returned to the parent** (per the directive above); the parent reads the bounded snippet only for candidates that survive the Step 4 budget cut. The named `ywc-architect` agent — and any Tier-2 language reviewer substituted for Design/Devex per the dispatch table below — has no `Write` tool: per the §3.5 exception in [`../references/subagent-status-actions.md`](../references/subagent-status-actions.md), it returns both categories **inline** instead of to a file.
...
4.5. **Independent Verification Pass** — For every **Confirmed finding** (not Advisor candidate) with severity Critical or High from **any** Step 3 aspect — Architecture, Design, Devex, QA, or **Security** — spawn one `model: sonnet` subagent per finding to attempt independent reproduction before the finding is presented as fact. This is an **independent Sonnet reproduction check, not the Advisor Pattern** (see [Advisor Pattern](#advisor-pattern) above) — these calls never count against `--advisor-budget`. Security's Confirmed findings are in scope here even though Security itself runs at Opus in Phase 1: the verifier is always Sonnet, independent of which model produced the original finding. Apply [../references/subagent-async-monitoring.md](../references/subagent-async-monitoring.md) for roster reconciliation across this up-to-20-way verifier dispatch.

   **Bounded dispatch payload**: the verifier subagent's prompt contains only the finding's `file:line` and its claimed severity — never the original finding's description or rationale text, to avoid anchoring bias. Inject the Step 3 return-payload contract verbatim. The verifier independently reads the cited location and reports either its own one-line description of the defect it found there (for semantic cross-check against the original) or "could not reproduce."

   **Dispatch cap**: verifier dispatch is capped at 20 findings per invocation (Critical and High combined). Order Critical before High, and within a tier preserve Step 3's discovery order. Findings beyond the cap are not dispatched; report them as `⚠️ Unverified — verifier dispatch cap reached` alongside the verified set, never as a plain Confirmed claim.

   - **Reproduced** — annotate the finding with a `✅ Verified` marker (reusing the existing status-marker vocabulary from [symbols.md](../references/symbols.md) §2 — do not invent a new symbol) and carry it to Step 6 unchanged.
   - **Could not reproduce** — never silently drop it (same discipline as Step 3's Fallback rule). Tag it `verification-failed`. For **Architecture / Design / Devex / QA** findings, add it to the Step 4 candidate pool for Phase 2, subject to the same `--advisor-budget` cap and Critical > High > Medium ordering — within a severity tier, `verification-failed` findings are prioritized ahead of same-severity Advisor candidates, since a failed independent reproduction is a stronger false-positive signal than a case Phase 1 was merely unsure about. Findings pushed out by the cap are dropped and logged exactly as Step 4 already logs over-budget drops. For **Security** findings, do **not** route to the Step 4 pool — Security already runs at Opus in Phase 1 and Step 4 explicitly excludes it (no higher-tier model to escalate to, same invariant as the "never produces Advisor candidates" rule above). Instead mark it `⚠️ Unverified — could not independently reproduce, no Phase 2 escalation path for Security` and surface it for manual review in the Step 6 report.
   - **Verifier execution failure** — a verifier call that times out, errors, returns a malformed response, or leaves its findings artifact missing is a distinct outcome from "could not reproduce": tag it `verification-error` (never conflate with `verification-failed`, which means the verifier ran and found nothing). For Architecture / Design / Devex / QA, route it into the same Step 4 candidate pool as `verification-failed` when Phase 2 is enabled; under `--no-advisor`, mark it `⚠️ Unverified — Phase 2 disabled via --no-advisor` the same way a failed reproduction is marked. For Security, apply the same no-escalation-path handling as `verification-failed` above — mark it `⚠️ Unverified — verifier execution failed, no Phase 2 escalation path for Security`. Count `verification-error` outcomes in the Step 6 report's verification summary separately from `Reproduced` and `Could not reproduce`.
   - **`--no-advisor` interaction** — Step 4.5 still runs (it is independent of the Advisor Pattern), but a finding that fails to reproduce cannot be escalated to a now-skipped Phase 2. Mark it `⚠️ Unverified — Phase 2 disabled via --no-advisor` in the report instead of presenting it as a flat Confirmed claim. (Security findings already have no escalation path regardless of `--no-advisor` — see above.)
...
- **Evidence quality** — Every finding must cite a verified source: file path, line number, or test output. Findings phrased as "this *might* be wrong" without primary evidence reduce the score even if the finding itself is plausible. When a task declares a Quality Gate Contract, a score cap applies only if that contract's corresponding `gate_state` (see [../references/quality-gates.md](../references/quality-gates.md) §9) is `"not run — tool unavailable"` or `"not run — dispatch failed"`; never when `gate_state` is `"not run — contract absent"` (the field-level `N/A — no quality gate contract` sentinel), so a contract-less project is not permanently held below the REVIEW band.
