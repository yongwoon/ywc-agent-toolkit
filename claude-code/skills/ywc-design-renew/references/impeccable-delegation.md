...
( ls -d ~/.claude/skills/impeccable .agents/skills/impeccable \
       .claude/skills/impeccable 2>/dev/null | head -1 ) \
  || command -v impeccable 2>/dev/null \
  || npx --no-install impeccable --version 2>/dev/null
```

- If a skill directory exists → delegate via the **skill invocation**
  (`/impeccable <mode>`), the richest path.
...
|---|---|---|---|
| `--mode check` (audit, no edits) | `/impeccable audit <scope>` | `npx impeccable detect <path>` | Grep Part B/C signals + AI Slop Test (anti-slop-ruleset.md) |
| `--mode renew`, new surface | `/impeccable craft <feature>` | — (no rewrite) | Apply Part D DO rules from scratch toward a bold direction |
| `--mode renew`, existing surface | `/impeccable polish <scope>` | — (no rewrite) | Apply Part D fixes to existing code, removing Part B/C tells |
| Component/token extraction | `/impeccable extract <target>` | — | Out of scope — note and skip |
