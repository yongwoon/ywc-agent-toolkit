...
A Claude Code Skill that renews generic or "AI-made" (AI-slop) frontend surfaces
...
- (Optional) `impeccable` skill — delegated to as a stronger design engine when
  present; self-contained ruleset fallback otherwise. **Install (either)**: in
  Claude Code run `/plugin marketplace add pbakaus/impeccable`, or
  `npx impeccable skills install`. After installing, run `/impeccable init` once
  to set the project Design Context — it writes the `PRODUCT.md` / `DESIGN.md`
  files below so the context questions are skipped.
