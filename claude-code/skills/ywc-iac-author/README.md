...
Infrastructure-as-Code 작성 Skill 입니다. Infrastructure 설계(`ywc-infra-design` 산출물) 또는 명시적으로 정리한 inline intent 를 바탕으로 Terraform 을 작성하고, Module 단위로 `ywc-cloud-engineer` Worker 에게 fan-out 한 뒤 `terraform validate` / `terraform plan` 으로 검증하고, apply 전에 blast-radius summary 를 보고합니다. 본 toolkit 에서 **Terraform 은 유일하게 고정된 IaC Tool** 이며, Kubernetes/Helm Resource 도 Terraform 의 `kubernetes` / `helm` Provider 로 표현합니다 — raw manifest 나 별도 IaC Tool 은 사용하지 않습니다.
...
- (선택) `--scope <module-path>` — 특정 Terraform Module 로 작성/fan-out 범위 제한 (default: 설계에서 파생되는 모든 Module)
...
- `ywc-cloud-engineer` — 각 Terraform Module 을 작성/검증하는 fan-out Worker
