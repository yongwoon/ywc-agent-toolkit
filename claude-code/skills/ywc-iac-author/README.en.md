...
Infrastructure-as-Code authoring skill. Turns an infrastructure design (or clarified inline intent) into Terraform, fans out per-module authoring to the `ywc-cloud-engineer` worker, verifies with `terraform validate` / `terraform plan`, and reports a blast-radius summary before anything is applied. **Terraform is the single fixed IaC tool** for this toolkit — Kubernetes/Helm resources are expressed through the Terraform `kubernetes` / `helm` providers, never raw manifests or a second IaC tool.
...
- (optional) `--scope <module-path>` — restrict authoring/fan-out to a single Terraform module (default: every module implied by the design)
...
- `ywc-cloud-engineer` — the fan-out worker that authors and verifies each Terraform module
