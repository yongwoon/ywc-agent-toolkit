...
Behavior changes are test-first: a bugfix writes a regression test that fails before the fix, and new behavior writes a failing test before implementation (docs/config/mechanical tasks are the exception). When a public contract changes, the interface is written before the body (deep module). When a task's Ownership matches a critical path (auth, payment, crypto, PII, external input), `/ywc-impl-review` and `/ywc-security-audit` are forced even without `--review`. See `../references/tdd-deep-module-gray-box.md` for details.
