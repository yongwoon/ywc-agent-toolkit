...
> **Action required**: [references/external-url-policy.md](./references/external-url-policy.md) is the **complete** procedure — every branch, the jsonc schema, the exact prompt wording, the Completion Report lines, and the rationale. Read it before Pre-flight; the summary below is an index into it, not a second source of truth.
