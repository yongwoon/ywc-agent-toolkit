...
Task ID 의 `[INITIALS]` 접두는 문자열 prefix 로 매칭되므로, legacy 무접두 ID (`000001-010-db-create-users-table`) 도 그대로 동작합니다.
...
| `--non-interactive` | 이번 run 의 interactive prompt 를 skip 합니다. External URL Policy 에 저장된 결정이 없으면 묻지 않고 `deny` 를 적용하며 저장하지 않습니다 (이번 run 한정). Step 4.5 의 `/ywc-impl-review` 자동 호출에도 그대로 전달됩니다. 독립 flag, 4종 delivery mode·`--review`·`--dry-run`·`--worktree` 와 결합 가능 | `--non-interactive` |
