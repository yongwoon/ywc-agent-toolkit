...
- Tier 3 (specialist) — 現在 5 種が land 済: Architect (Opus) / Security Engineer (Opus) / Refactor Cleaner (Sonnet) / Root-cause Analyst (Opus) / Performance Engineer (Sonnet)。追加 specialist は follow-up PR
...
| `ywc-security-engineer` | 3 (specialist) | opus | Static security review worker (read-only)。OWASP Top 10 + threat modeling + secret/PII scan、severity-rated findings + 具体 remediation。CRITICAL 重大度が merge-blocking であるため Phase 1 から Opus 単一 pass (Phase 2 escalation なし)。ywc-security-audit / ywc-impl-review Phase 1 / ywc-incident-postmortem から dispatch |
