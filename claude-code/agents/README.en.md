...
- Tier 3 (specialist) — 5 agents currently shipped: Architect (Opus) / Security Engineer (Opus) / Refactor Cleaner (Sonnet) / Root-cause Analyst (Opus) / Performance Engineer (Sonnet); additional specialists — follow-up PR
...
| `ywc-security-engineer` | 3 (specialist) | opus | Static security review worker (read-only). OWASP Top 10 + threat modeling + secret/PII scan, severity-rated findings with concrete remediation. Runs at Opus from Phase 1 (no Phase 2 escalation) because a missed or misjudged finding is CRITICAL severity and merge-blocking. Dispatched from ywc-security-audit / ywc-impl-review Phase 1 / ywc-incident-postmortem |
