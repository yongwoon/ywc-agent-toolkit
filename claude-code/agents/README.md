...
- Tier 3 (specialist) — Architect (Opus) / Security Engineer (Opus) / Refactor Cleaner (Sonnet) / Root-cause Analyst (Opus) / Performance Engineer (Sonnet) 5종 운영, 추가 specialist 는 추후 follow-up PR
...
| `ywc-security-engineer` | 3 (specialist) | opus | Static security review worker (read-only). OWASP Top 10 + threat modeling + secret/PII scan, severity-rated findings + 구체 remediation. CRITICAL 심각도 판정이 merge-blocking 이므로 Opus 단일 pass (Phase 2 escalation 없음). ywc-security-audit / ywc-impl-review Phase 1 Security subagent / ywc-incident-postmortem 에서 dispatch |
