...
Toolkit 자체의 `ywc-*` Skill 과 Agent 품질을 **등급(0~5, 가중 100점)으로 채점**하고, `평가 → 점수 → 우선순위 → 개선 → 재평가` 사이클을 구동하는 Meta Skill 입니다. `ywc-skill-author` 가 제작 시점의 **이진 규칙**을 정의한다면, 이 Skill 은 이미 존재하는 Skill/Agent 의 **graded scorecard** 를 만들어 약한 항목부터 고치게 합니다.
