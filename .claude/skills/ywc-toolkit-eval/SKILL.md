...
Resolve `--target` to a concrete item list. For `all`, score `claude-code/skills`, `claude-code/agents`, and `codex/skills`. Read `evals/history.json` (if present) to obtain the previous scores for regression comparison.
